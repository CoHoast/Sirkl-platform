  ...
  try {
    ...
    if (!negId) {
      const negResult = await pool.query(`
        INSERT INTO negotiations (bill_id, initial_offer, current_offer, strategy, status, created_at, updated_at)
        VALUES ($1, $2, $2, 'medicare_percentage', 'pending', NOW(), NOW())
        RETURNING id
      `, [billId, offerAmount]);
      ...
      await pool.query(`
        UPDATE negotiations SET current_offer = $1, updated_at = NOW() WHERE id = $2
      `, [offerAmount, negId]);
    }
